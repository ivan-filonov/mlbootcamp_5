...
def add_transformed(train, test, base_columns, clf, cname):
    train_ = clf.fit_transform(train[base_columns])
    test_ = clf.transform(test[base_columns])
    for i in range(train_.shape[1]):
        train[cname + str(i)] = train_[:, i]
        test[cname + str(i)] = test_[:, i]
    print(cname, 'feature columns added')
    del train_, test_, i
...
def cleanup_and_generate(train, y, test, random_state=1234):
    ...
    df_all['w_div_h_s100'] = df_all.weight / np.maximum(df_all.height - 100, 1)
    df_all['h_sub_w'] = df_all.height - df_all.weight
    df_all['ap_hi_sub_w'] = df_all.ap_hi - df_all.weight
    df_all['ap_lo_sub_w'] = df_all.ap_lo - df_all.weight
    df_all['ap_diff_div_w'] = df_all.ap_diff / (df_all.weight + 1)
    ...
    #'''
    base_columns = ['id', 'age', 'r_weight', 'r_height', 'r_ap_hi', 'r_ap_lo',
                    'cholesterol', 'gluc', 'active', 'alco', 'smoke']
    for ncl in range(3, 50, 5):
    ...
    for c in base_columns:
        skf = model_selection.StratifiedKFold(n_splits=15, shuffle=True, random_state=11)
        cname = c + '_wa'
        train2[cname] = 0
        test2[cname] = 0
...
if '__main__' == __name__:
    ...
    z.y = (
            (z.xgb1 + z.xgb2 + z.xgb3) * (0.9 / 3) +
            (z.keras1 + z.rf1 + z.et1) * (0.1 / 3)
        )
    z.y = prestore(z.y)
    save_results(v, z)
    ...
    #'''
    clf = xgb.XGBClassifier(n_estimators=1500, learning_rate=.05)
    clf.fit(train2, y)

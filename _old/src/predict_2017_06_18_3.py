...
def cleanup_and_generate(train, y, test, use_hand_labels = False):
    ...
    base_columns = set(df_all.columns) - {'id', 'y'}
    tr = pd.concat([df_all[:ntrain], y], axis=1)
    for c in base_columns:
        cname = c + '_wa'
        wd = weighted_average_dict(tr, c, 'cardio')
        df_all[cname] = df_all[c].map(wd)
    df_all.fillna(y.mean(), inplace=True)
...
if '__main__' == __name__:
    ...
    #z.y = z.xgb1 * 0.45 + z.xgb2 * 0.45 + (z.rf1 + z.et1 + z.keras1) * (0.1 / 3)
    z.y = z.et1
    z.y = prestore(z.y)
    save_results(v, z)

...
def xgb_base(train2, y, test2, v, z, xgb_params, N_splits, N_seeds, cname, base_seed=42):
    ...
    for s in range(N_seeds):
        ...
        for n, (itrain, ival) in enumerate(skf.split(train2, y)):
            ...
            mtrain = clf.predict(dtrain, output_margin=True)
            mvalid = clf.predict(dvalid, output_margin=True)
            mtest = clf.predict(dtest, output_margin=True)

            dtrain.set_base_margin(mtrain)
            dvalid.set_base_margin(mvalid)
            dtest.set_base_margin(mtest)

            xgb_params['seed'] = s + base_seed + 1
            clf = xgb.train(xgb_params, dtrain, 1, watch)
...
def keras_mlp1(train2, y, test2, v, z):
    ...
    num_splits = 9
    ss = model_selection.StratifiedKFold(n_splits=num_splits, random_state=11)
...
if '__main__' == __name__:
    ...
    if 'keras_mlp1' not in v.columns:
        keras_mlp1(train2, y, test2, v, z)
        save_state('model_predicts', (v, z))
    if 'keras_resnet1' not in v.columns:
        keras_resnet1(train2, y, test2, v, z)
        save_state('model_predicts', (v, z))

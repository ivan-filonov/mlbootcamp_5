...
def tune_xgb(train2, y, test2, v, z):
    ...
    cv = xgb.cv(xgb_params, dtrain, nfold=10, num_boost_round=10000, early_stopping_rounds=100, seed=123)
    ...
    v[cname], z[cname] = 0, 0
    N_splits = 9
    # 1. xtrain->predict ytrain to v
    scores = []
    skf = model_selection.StratifiedKFold(n_splits=N_splits, shuffle=True)
    for n, (itrain, ival) in enumerate(skf.split(train2, y)):
        dtrain = xgb.DMatrix(train2.ix[itrain], label=y[itrain])
        dvalid = xgb.DMatrix(train2.ix[ival], label=y[ival])
        watch = [(dtrain, 'train'), (dvalid, 'valid')]
        clf = xgb.train(xgb_params, dtrain, 10000, watch, early_stopping_rounds=100, verbose_eval=False)
    ...
    # 2. xtest->predict z
    dtrain = xgb.DMatrix(train2, label=y)
    dpred = xgb.DMatrix(test2)
    cv = xgb.cv(params=xgb_params,
                dtrain=dtrain,
                num_boost_round=10000,
                early_stopping_rounds=100,
                nfold=10,
                stratified=True)
    print(cname, 'num_boost_rounds =', len(cv))
    print(cv)
    bst = xgb.train(params=xgb_params, dtrain=dtrain, num_boost_round=len(cv))
    z[cname] = pconvert(bst.predict(dpred))

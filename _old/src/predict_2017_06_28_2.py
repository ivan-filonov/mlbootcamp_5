...
def restore_missing_xgb(df, N_splits = 10):
    xgb_params = dict(
            max_depth = 4,
            learning_rate = 0.005,
            subsample = 0.7,
...
def restore_missing(df, N_splits = 10):
    df.alco.replace(-1, 0, inplace=True)
    df.smoke.replace(-1, 0, inplace=True)
    df.active.replace(-1, 1, inplace=True)
...
def tune_xgb(train2, y, test2, v, z):
    ...
    cv = xgb.cv(xgb_params, dtrain, nfold=10, num_boost_round=10000, early_stopping_rounds=1000, seed=1)
    print(cname, len(cv))
    ...
    xgb_params['learning_rate'] = 0.004

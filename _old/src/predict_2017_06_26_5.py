...
def restore_missing(df):
    ...
    print('before restore loop')
    for label in ['active', 'smoke', 'alco']:
        ...
        skf = model_selection.StratifiedKFold(n_splits=N_splits, shuffle=True, random_state=1)
        for n, (itrain, ival) in enumerate(skf.split(xtrain, ytrain)):
            #print(ytrain.ix[itrain].value_counts(), ytrain.ix[ival].value_counts())
            dtrain = xgb.DMatrix(xtrain.ix[itrain], ytrain[itrain])
            dvalid = xgb.DMatrix(xtrain.ix[ival], ytrain[ival])
            watch = [(dtrain, 'train'), (dvalid, 'valid')]
            clf = xgb.train(xgb_params, dtrain, 10000, watch, early_stopping_rounds=100, verbose_eval=1000)
            ypred += clf.predict(dpred)
        ypred /= N_splits
        df.ix[df[label] == -1, label] = (ypred > 0.5) * 1
        print('restored', label, df[label].value_counts())

def fixed_cleanup(df_all):
    ...
    #df_all.ix[idx, ['height', 'weight']] = 175, 167
    #--------------------7----------------------------
...
def cleanup_and_generate(train, y, test, random_state=1234):
    ...
    df_all['ap3'] = df_all.ap_lo * 2 + df_all.ap_hi
    df_all['ap3_div_h'] = df_all.ap3 / h
    df_all['ap3_div_w'] = df_all.ap3 / w

    df_all['h_sub_w'] = df_all.height - w
    df_all['h_sub_aph'] = df_all.height - df_all.ap_hi
    df_all['h_sub_apl'] = df_all.height - df_all.ap_lo
